...
topical_09 <- function(file_path) {
  ...
  col_specs <- fwf_positions(
    start = c(6, 45, 806, 696, 676, 22, 736),
    end   = c(17, 48, 815, 705, 685, 23, 745),
    col_names = c("SSUID", "PNUM", "TPRLOANAMT", "THVAL_HOME", "THNETWORTH", "WAVE", "THVAL_BANK")
  )
...
topical_10 <- function(file_path) {
  # Define column specifications with explicit start & end positions
  col_specs <- fwf_positions(
    start = c(6, 45, 798, 688, 668, 22, 728),
    end   = c(17, 48, 807, 697, 677, 23, 737),
    col_names = c("SSUID", "PNUM", "TPRLOANAMT", "THVAL_HOME", "THNETWORTH", "WAVE", "THVAL_BANK")
  )
